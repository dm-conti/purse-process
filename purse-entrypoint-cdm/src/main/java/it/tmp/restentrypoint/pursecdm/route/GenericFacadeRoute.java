 ...
 * 
 */
public class GenericFacadeRoute extends RouteBuilder {

	private static final String FACADE_CLASS = "it.tmp.restentrypoint.pursecdm.service.GenericFacadeService";
	private static final String ROUTE_ID = "GenericFacadeRoute";
	...
	public void configure() throws Exception {
		from("cxfrs:bean:genericServer?bindingStyle=SimpleConsumer&resourceClasses=" + FACADE_CLASS)
		.routeId(ROUTE_ID)
...
//					.inOut("mock:activemq:queue:" + Queues.PURSE_QUEUE.inputQueue() + "?replyTo=restChannel.out")
				...
				.when(header("operationName").isEqualTo("postSomething"))
					/**
					 * TitanMessageBuildProcessor call TitanMessageBuilder and after invocation
					 * we have valorized exchange.body whit TitanMessage
					**/
					.process(TitanMessageBuildProcessor.getInstance().setHeader(MessageHeaders.REPLY_CHANNEL, REPLY_CHANNEL))
					.to("log:input?level=" + ROUTE_LOGGING_LEVEL.name() + "&showAll=true&multiline=true")
					/**
